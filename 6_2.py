...
class SimParams:
    ...
    def __init__(self, 
                 ...
                 num_boids=15):                  # number of boids in the simulation (int)
    ...
    def neighbors(self, params, boids):
        ...
        for boid in boids:
            if np.any(boid.pos != self.pos):
                ...
                boid_modified = Boid(np.array([boid.pos[i] for i in range(params.num_dims)]), boid.v)
                for i in range(params.num_dims):
                    if self.pos[i] + (params.dim_width[i]-boid.pos[i]) < params.radius:
                        boid_modified.pos[i] -= params.dim_width[i]
                    if (params.dim_width[i]-self.pos[i]) + boid.pos[i] < params.radius:
                        boid_modified.pos[i] += params.dim_width[i]

                dist = np.linalg.norm(boid_modified.pos - self.pos)

                ang = np.arccos(np.clip(np.dot((boid_modified.pos - self.pos)/np.linalg.norm(boid_modified.pos - self.pos), self.v/np.linalg.norm(self.v)), -1.0, 1.0))
...
def update_frame(frame, params, scatter, boids, order_text, step_text, hist_ax):
    ...
    order = (1/params.num_boids) * np.linalg.norm(sum([boid.v / np.linalg.norm(boid.v) for boid in boids]))
    order_text.set_text(f'Order: {order}')
    step_text.set_text(f'Number of steps: {frame}')

    # Plot histogram of nearest neighbor distances
    hist_ax.clear()
    hist_ax.hist(nn_distances, bins=np.arange(0, 601, 25), color='skyblue', edgecolor='black')
...
def run_simulation(params, show_anims=True):
    boids = [Boid(np.array([random.uniform(0, params.dim_width[i]) for i in range(params.num_dims)]), 
    ...
    if show_anims:
        ...
        scatter = ax_scatter.scatter([boid.pos[0] for boid in boids], [boid.pos[1] for boid in boids])
        ...
        ani = FuncAnimation(fig, update_frame, frames=params.num_steps, fargs=(params, scatter, boids, order_text, step_text, ax_hist), interval=1)
        plt.xlim([0, 600])
        plt.ylim([0, 600])
        plt.show()
    else:
...
def run_ABC(N, thresholds, cov, ex_6_4=False):
    ...
    while len(populations[0]) < N:
        ...
        cand = {'cohesion': random.uniform(0, .1), 
                'separation': random.uniform(0, .1),
                'alignment_strength': random.uniform(0, .1)}
        performance = run_simulation(SimParams(cohesion=cand['cohesion'], 
                                               separation=cand['separation'], 
                                               alignment_strength=cand['alignment_strength']), show_anims=False)
        
        

        if ex_6_4:
            performance = 2 / (1 + np.exp(5 * abs(performance - 0.6)))

        cand['score'] = performance


        if performance > thresholds[0]:
            print(f"...Got a new member with fitness: {performance}")
            populations[0].append(cand)
        else:
            print(f"...Failed a candidate with fitness: {performance}")
...
populations = run_ABC(20, [0.95], 0.005, ex_6_4=True)
