...
def run_simulation(params, show_anims=True):
    ...
    if show_anims:
        ...
        ani = FuncAnimation(
            ...
            fargs=(params, scatter, boids, order_text, step_text, ax_hist),
            interval=1,
        )
        plt.xlim([0, 600])
        plt.ylim([0, 600])
...
def run_ABC(N, thresholds, cov, ex_6_4=False):
    ...
    while len(populations[0]) < N:
        ...
        if performance > thresholds[0]:
            print(f"...Got a new member with fitness: {performance} and order: {order}")
            populations[0].append(cand)
        else:
            print(
                f"...Failed a candidate with fitness: {performance} and order: {order}"
            )
    ...
    for i, threshold in enumerate(thresholds[1:]):
        ...
        while len(new_population) < N:
            ...
            if performance > threshold:
                ...
                print(
                    f"...Got a new member with fitness: {performance} and order: {order}"
                )
            else:
                print(
                    f"...Failed a candidate with fitness: {performance} and order: {order}"
...
populations = run_ABC(20, list(np.arange(0.5, 1, 0.05)) + [0.99], 0.005, ex_6_4=True)
with open("results_6.4.json", "w") as f:
